        ...
        <div className="fixed inset-0 z-50 flex">
          ...
          <div className="relative flex flex-col w-full max-w-xs bg-theme-bg border-r border-theme-border h-full shadow-2xl overflow-y-auto z-10 p-6 animate-in slide-in-from-left duration-200">
            ...
            <div className="flex items-center justify-between pb-4 mb-6 border-b border-theme-border">
              <h3 className="text-xs font-bold text-(--line-color-600) dark:text-(--line-color-400) uppercase tracking-widest">
                {line.title}
              </h3>
              <button
                onClick={() => setIsOpen(false)}
                className="p-2 text-theme-muted hover:text-theme-text font-bold text-lg cursor-pointer"
              >
                ✕
              </button>
            </div>
              ...
              {posts.map((p, index) => {
                ...
                return (
                  <div key={p.slug} className="mb-4 ml-6 relative">
                    ...
                    <div
                      className={`absolute -left-9 top-1 w-4 h-4 rounded-full border-2 bg-theme-bg transition-all duration-300
                        ${isCurrent ? "scale-150 border-(--line-color-600)" : "border-slate-500"}`}
                    />

                    <Link
                      href={`/post/${line.trail.slug}/${line.slug}/${p.slug}`}
                      onClick={() => setIsOpen(false)} // Fecha o menu automaticamente ao mudar de post
                      className={`text-sm transition-colors flex items-center group w-full min-w-0 overflow-hidden
                        ${isCurrent ? "font-bold text-(--line-color-600) dark:text-(--line-color-400)" : 
